...
(defun walk-through-storm (blizzards min-pos max-pos start-pos end-pos &optional (start-minute 0))
  (loop named outer
        with visited = (make-hash-table :test 'equal)
        with blizzard-states = (make-hash-table)
        with queue = (make-queue :simple-queue)
        initially (qpush queue (list start-minute start-pos))
                  (setf (gethash start-minute blizzard-states) blizzards)
        ...
        do (loop for neighbor-delta in *neighbor-deltas*
                 ...
                 when (equal next-pos end-pos)
                   do (return-from outer (values next-minute next-blizzards))
...
(defun task1 (inputs)
  (multiple-value-bind (blizzards min-pos max-pos start-pos end-pos)
      (analyze-inputs inputs)
    (nth-value 0 (walk-through-storm blizzards min-pos max-pos start-pos end-pos))))

(defun task2 (inputs)
  (multiple-value-bind (blizzards min-pos max-pos start-pos end-pos)
      (analyze-inputs inputs)
    (let ((current-minute))
      (multiple-value-setq (current-minute blizzards)
        (walk-through-storm blizzards min-pos max-pos start-pos end-pos))
      (multiple-value-setq (current-minute blizzards)
        (walk-through-storm blizzards min-pos max-pos end-pos start-pos current-minute))
      (nth-value 0 (walk-through-storm blizzards min-pos max-pos start-pos end-pos current-minute)))))
